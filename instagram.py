...
def select_to_Parse(cursor):
	...
	global parse_follower_length
	parse_follower_length=len(toParseList)
...
def  get_user_to_parse_photos(cursor):
	...
	global parse_photo_lenth
	parse_photo_lenth = len(toParsePhotoList)
...
def save_photos(photo_url,real_dir):
	...
	try:
		content = requests.get(photo_url,headers = headers(photo_url),timeout=10).content
	except:
		return 
...
def main():
	...
	#配置
	service_args = [
	# 代理 IP：port    （eg：192.168.0.28:808）
	...
	do_time = (end_time - start_time).hours
	with open('/home/luojunjie/python/instagram/log.txt','w+',encoding='utf-8') as f:
		f.write('{}至{}爬取了{}个用户的关注者,{}个用户的照片,{}张照片,共耗时{}个小时 \n'.format(start_time,end_time,parse_follower_length,parse_photo_lenth,photo_count,do_time))
